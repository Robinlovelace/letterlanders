...
use letterlanders_core::{GameEngine, GameStatus, GameVariant, SoundEvent, GameSettings};
...
fn run_app(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, sink: Option<&rodio::Sink>, settings: GameSettings) -> Result<()> {
                    ...
                    match engine.status {
                        GameStatus::Menu => {
                            match key.code {
                                ...
                                KeyCode::Char('s') | KeyCode::Char('S') => engine.status = GameStatus::Settings { message: None },
                        ...
                        GameStatus::Settings { ref mut message } => {
                            ...
                            match key.code {
                                KeyCode::Char('1') => {
                                    engine.settings.show_target_visual = !engine.settings.show_target_visual;
                                    *message = None;
                                }
                                KeyCode::Char('2') => {
                                    engine.settings.feedback_duration_seconds = match engine.settings.feedback_duration_seconds {
                                        1 => 2,
                                        2 => 3,
                                        3 => 5,
                                        _ => 1,
                                ...
                                KeyCode::Char('3') => {
                                    engine.settings.input_method = match engine.settings.input_method {
                                ...
                                KeyCode::Char('6') => {
                                    ...
                                    match GameSettings::load_from_file("letterlanders_settings_export.json") {
                                        ...
                                        Err(_) => *message = Some("Import failed: file not found or invalid".to_string()),
                                    }
                                }
                                KeyCode::Esc => {
                        ...
                        GameStatus::Feedback { .. } => {
                            match key.code {
                                // KeyCode::Enter => engine.next_level(), // Removed manual advance
                                KeyCode::Esc => engine.status = GameStatus::Menu,
                                _ => {}
                            }
                        }
